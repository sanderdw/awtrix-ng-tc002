...
static void stack_resize(bvm *vm, size_t size)
{
    ...
    /* AWTRIX (candidate for upstream): nil-fill the grown tail; see the note
     * at the stack allocation in be_vm.c -- premark_stack may scan slots that
     * no instruction has written yet. */
    if (sizeof(bvalue) * size > os)
        memset((char*)vm->stack + os, 0, sizeof(bvalue) * size - os);
