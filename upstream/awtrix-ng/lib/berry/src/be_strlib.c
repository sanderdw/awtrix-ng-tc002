...
/* AWTRIX: newlib-nano's `_printf_i` has no 64-bit integer conversion -- see
** scripts/newlib_nano.py, which links nano for the ~52 KB of flash it saves.
** Handing it BE_INT_FORMAT ("%lld" at BE_INTGER_TYPE 2) prints the literal
** "ld" instead of the number, and Berry stringifies integers everywhere, so the
** conversion happens here. `prec` is the minimum digit count, zero-padded; 0
** means "however many the value needs". */
static void int2str(char *buf, size_t size, bint value, int base, int upper, int prec)
{
    static const char lower[] = "0123456789abcdef";
    static const char upper_[] = "0123456789ABCDEF";
    const char *alpha = upper ? upper_ : lower;
    char digits[24];
    ...
    if (base == 10) {
        if (value < 0) {
            neg = 1;
            u = (uintmax_t)0 - u; /* negate unsigned: -M_IMIN overflows a bint */
        }
    } else {
        /* %x and friends print the bint-wide two's complement, so drop the sign
        ** extension the cast to uintmax_t added. The shift is 0 when the two
        ** types are the same width, which leaves the mask all ones. */
        u &= (uintmax_t)~(uintmax_t)0 >> ((sizeof(uintmax_t) - sizeof(bint)) * 8);
    }
    do {
        digits[n++] = alpha[u % (uintmax_t)base];
        u /= (uintmax_t)base;
    } while (u != 0 && n < sizeof(digits));
    while (prec > 0 && n < (size_t)prec && n < sizeof(digits)) {
        digits[n++] = '0';
    }
    if (neg && out + 1 < size) {
        buf[out++] = '-';
    }
    while (n > 0 && out + 1 < size) {
        buf[out++] = digits[--n];
    }
    buf[out] = '\0';
}
...
bstring* be_num2str(bvm *vm, bvalue *v)
{
    char buf[25];
    if (var_isint(v)) {
        int2str(buf, sizeof(buf), var_toint(v), 10, 0, 0);
...
static bstring* sim2str(bvm *vm, bvalue *v)
{
    ...
    case BE_INDEX:
    case BE_INT:
        int2str(sbuf, sizeof(sbuf), var_toint(v), 10, 0, 0);
        break;
    case BE_REAL:
        ...
        for (;;) {
            ...
            case 'u': case 'x': case 'X':
            {
                ...
                if (convert_to_int(vm, index, &val)) {
                    /* AWTRIX: `long` is the widest integer nano converts, and it
                    ** does apply every flag, width and precision to it. A value
                    ** that does not fit keeps all its digits but loses the
                    ...
                    ** rather than 64-bit unsigned. See scripts/newlib_nano.py. */
                    if (val >= (bint)LONG_MIN && val <= (bint)LONG_MAX) {
                        ...
                        int2str(buf, sizeof(buf), val, base, *p == 'X', 0);
                    }
                }
...
static int str_i2hex(bvm *vm)
{
    ...
    if (top && be_isint(vm, 1)) {
        ...
        char buf[24];
        int prec = 0;   /* AWTRIX: was "%.*llX" -- see int2str above */
        ...
        int2str(buf, sizeof(buf), value, 16, 1, prec);
        be_pushstring(vm, buf);
        be_return(vm);
    }
    be_return_nil(vm);
