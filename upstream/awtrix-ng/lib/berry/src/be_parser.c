...
static void end_func(bparser *parser)
{
    ...
    /* AWTRIX: upstream also moves ktab here, but real constants are read with
     * FPU loads (LSI), which cannot access the ESP32's IRAM-only region --
     * proven by a LoadStoreError in vm_exec's DIV on the first script with a
     * float constant. Only arrays whose every access is a core 32-bit load
     * (code above, lineinfo below) may move. */
