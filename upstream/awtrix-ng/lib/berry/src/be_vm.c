...
BERRY_API bvm* be_vm_new(void)
{
    ...
    /* AWTRIX (candidate for upstream): the GC's premark_stack scans up to
     * vm->top, and precall raises top over frame slots no instruction has
     * written yet. On a freshly malloc'd stack those slots are garbage; if one
     * happens to look like a GC object the mark phase dereferences a wild
     * pointer. Nil-fill the fresh stack (and every growth tail, be_exec.c) so
     * every slot the GC can see is a valid value. */
    memset(vm->stack, 0, sizeof(bvalue) * BE_STACK_START);
